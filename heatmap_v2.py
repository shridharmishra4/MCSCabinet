...
def cal_cam(video, inter_data):
    ...
    for frame_ids, intersection in inter_data.items():  
        ...
        for i, record in enumerate(intersection):
            ...
            for hand in ["left", "right"]:
                ...
                for j, p1 in enumerate(p_list):          
                    ...
                    if 0 <= map_x < h_h and 0 <= map_y < h_w:
                        heatmap_i[h_h - map_x-1, map_y] = 1
                        heatmap_i = cv2.GaussianBlur(heatmap_i,(7,7),0)
                        heatmap += heatmap_i
                    else:
                        continue   
            ...
            if np.max(heatmap_i) > 0.01:
                heatmap_is = cv2.applyColorMap(np.uint8(255 * (heatmap_i/np.max(heatmap_i))), cv2.COLORMAP_JET)
            else:
                heatmap_is = np.zeros((h_h, h_w, 3), dtype="uint8")
            heatmap_is[heatmap_i < 0.01] = 0
