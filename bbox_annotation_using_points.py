...
def annotate_corners(videopath):
    ...
    w,h = img.shape[:2][0],img.shape[:2][1]
    # org
    org = (80, 50)
    org1 = (380, 100)
    ...
    p = get_points_from_csv(videopath)
    pts = p
    plot_points(text_img,p)

    image = cv2.putText(text_img, 'Save previous annotations?', org, font, fontScale, color, thickness, cv2.LINE_AA)
    image = cv2.putText(image, 'Press Y', org1, font, fontScale, color, thickness, cv2.LINE_AA)

    cv2.imshow('image',image)
    key = cv2.waitKey()
    if key == ord('y'):
        filename = videopath.split('.')[0] + '_bbox.csv'
...
if __name__ == "__main__":
    session = sys.argv[1]
    for item in os.listdir(f"{session}/top"):
        if item.endswith(".mp4"):
            annotate_corners(f"{session}/top/"+item)
