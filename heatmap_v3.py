...
# Normalised based on dimensions of the cabinet
...
def load_bbox_from_file(file_name):
    ...
    :param file_name: file with coodinates of bounding box
    :return: dict of 4 corners
    """
    data = {}
    with open(file_name, 'r') as f:
        for i, line in enumerate(f):
...
# Legacy hardcoded bounding box coordinates of front of bounding box
# DATA = {
...
def read_track(top, front):
    """
    Reads hand tracks from top view videos and front view videos
    :param top:
    :param front:
    :return:
    """
...
def check_parameters(K, dist, R, T, p_gt, p3d_gt):
    """
    Verifies if projected parameters are correct
    :param K:
    :param dist:
    :param R:
    :param T:
...
def get_hand_pose(data, handpose, ratio_x=1, ratio_y=1):
    """
    Gets hand pose coordinates from pickle dump
    Hand pose generated using https://google.github.io/mediapipe/solutions/hands.html
    :param data:
    :param handpose:
    :param ratio_x:
    :param ratio_y:
...
def get_track(video, top_data, front_data, save=True):
    ...
    for i, frames in enumerate(top_data.keys()):
        ...
        for count in range(frame_number):
            ...
            record = {"left": intersect_left, "right": intersect_right, "pos_left": front_left_, "top_left": top_left,
                      "top_right": top_right,
...
def cal_cam(video, inter_data):
    ...
    for frame_ids, intersection in inter_data.items():
        ...
        # calibrate cameras
        err, mtx1, dist, R1, T1 = cv2.calibrateCamera([objpoints1], [imgpoints1], dim, mtx1, None,
        ...
        cap = cv2.VideoCapture(video)
        output_cap = cv2.VideoWriter(dirs + name + '_{}_{}_heat.mp4'.format(start, end), fourcc, int(fps),
                                     (h_w * 4, h_h))
        output_cap_cummulative = cv2.VideoWriter(dirs + name + '_{}_{}_heat_cummulative.mp4'.format(start, end), fourcc,
                                                 int(fps), (h_w * 3, h_h))
        ...
        for i, record in enumerate(intersection):
            ...
            if front_right is not None:
                if record["right"]:
                    cv2.rectangle(frame, (front_right[0], front_right[1]), (front_right[2], front_right[3]),
                                  (0, 255, 0), 2)
            ...
            if np.max(frame_heatmap) > 0.01:
                heatmap_is = cv2.applyColorMap(np.uint8(255 * (frame_heatmap / np.max(frame_heatmap))),
                                               cv2.COLORMAP_JET)
            else:
            ...
            if np.max(left_frame_heatmap) > 0.01:
                heatmap_left_coloured = cv2.applyColorMap(
                    np.uint8(255 * (left_frame_heatmap / np.max(left_frame_heatmap))), cv2.COLORMAP_JET)
            else:
            ...
            if np.max(right_frame_heatmap) > 0.01:
                heatmap_right_coloured = cv2.applyColorMap(
                    np.uint8(255 * (right_frame_heatmap / np.max(right_frame_heatmap))), cv2.COLORMAP_JET)
            else:
...
def draw_inter(video, inter_data):
    ...
    for frame_ids, inter in inter_data.items():
        cap = cv2.VideoCapture(video)
        start, end = eval(frame_ids)
        output_front = cv2.VideoWriter(dirs + name + '_{}_{}_front.mp4'.format(start, end), fourcc, int(fps),
                                       (w - limit_y, h))
        output_top = cv2.VideoWriter(dirs + name + '_{}_{}_top.mp4'.format(start, end), fourcc, int(fps),
                                     (limit_y, h - limit_x))
        ...
        for r in inter:
            ...
            cv2.rectangle(f_top, (TOP_BOX[current_clip][0], TOP_BOX[current_clip][1]),
                          (TOP_BOX[current_clip][2], TOP_BOX[current_clip][3]), (0, 0, 0), 2)
            ...
            if top_right is not None:
                ...
                cv2.putText(f_top, "RIGHT", (top_right[0], top_right[1]), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255),
                            2, cv2.LINE_AA)
            if top_left is not None:
                cv2.rectangle(f_top, (top_left[0], top_left[1]), (top_left[2], top_left[3]), cl, 2)
                cv2.putText(f_top, "LEFT", (top_left[0], top_left[1]), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2,
                            cv2.LINE_AA)
            if front_right is not None:
                cv2.rectangle(f_front, (front_right[0], front_right[1]), (front_right[2], front_right[3]), cr, 2)
                cv2.putText(f_front, "RIGHT", (front_right[0], front_right[1]), cv2.FONT_HERSHEY_SIMPLEX, 1,
                            (255, 255, 255), 2, cv2.LINE_AA)
            if front_left is not None:
                cv2.rectangle(f_front, (front_left[0], front_left[1]), (front_left[2], front_left[3]), cl, 2)
                cv2.putText(f_front, "LEFT", (front_left[0], front_left[1]), cv2.FONT_HERSHEY_SIMPLEX, 1,
                            (255, 255, 255), 2, cv2.LINE_AA)
